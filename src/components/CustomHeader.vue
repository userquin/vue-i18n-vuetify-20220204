<script setup lang="ts">
import { useGlobalI18n, useLocalI18n } from '@/composables/i18n'
const props = withDefaults(defineProps<{ global?: boolean }>(), { global: false })
const { t: tg } = useGlobalI18n()
const { t: tl } = useLocalI18n()
const t = (key: string) => {
  return props.global ? tg(`frameworks.${key}`) : tl(`frameworks.${key}`)
}
</script>

<i18n>
en:
  frameworks:
    vite: Vite [LOCAL SFC]
    ts: TypeScrypt [LOCAL SFC]
    vue: Vue [LOCAL SFC]
    vuetify: Vuetify [LOCAL SFC]
    unocss: UnoCSS [LOCAL SFC]
es:
  frameworks:
    vite: Vite [LOCAL SFC]
    ts: TypeScrypt [LOCAL SFC]
    vue: Vue [LOCAL SFC]
    vuetify: Vuetify [LOCAL SFC]
    unocss: UnoCSS [LOCAL SFC]
</i18n>

<template>
  <header>
    <span class="i-logos-vitejs" :title="t('vite')" />
    <span class="i-icon-park-plus-cross" />
    <span class="i-logos-typescript-icon" :title="t('ts')" />
    <span class="i-icon-park-plus-cross" />
    <span class="i-logos-vue" :title="t('vue')" />
    <span class="i-icon-park-plus-cross" />
    <span class="i-logos-vuetifyjs" :title="t('vuetify')" />
    <span class="i-icon-park-plus-cross" />
    <svg width="32" height="32" viewBox="0 0 220 220" fill="none" xmlns="http://www.w3.org/2000/svg">
      <title>{{ t('unocss') }}</title>
      <path
        d="M117.722 167.444C117.722 139.83 140.108 117.444 167.722 117.444V117.444C195.336 117.444 217.722 139.83 217.722 167.444V167.444C217.722 195.058 195.336 217.444 167.722 217.444V217.444C140.108 217.444 117.722 195.058 117.722 167.444V167.444Z"
        fill="#888" fill-opacity="0.6"
      />
      <path
        d="M117.722 52.5561C117.722 24.9419 140.108 2.55614 167.722 2.55614V2.55614C195.336 2.55614 217.722 24.9419 217.722 52.5561V97.5561C217.722 100.318 215.483 102.556 212.722 102.556H122.722C119.961 102.556 117.722 100.318 117.722 97.5561V52.5561Z"
        fill="#888" fill-opacity="0.3"
      />
      <path
        d="M102.278 167.444C102.278 195.058 79.8922 217.444 52.278 217.444V217.444C24.6637 217.444 2.27796 195.058 2.27796 167.444L2.27796 122.444C2.27796 119.682 4.51654 117.444 7.27796 117.444L97.278 117.444C100.039 117.444 102.278 119.682 102.278 122.444L102.278 167.444Z"
        fill="#888"
      />
    </svg>
  </header>
</template>

<style lang="scss" scoped>
header {
  display: grid;
  grid-template-columns: repeat(9, min-content);
  grid-column-gap: 0.5rem;
  justify-content: center;
  svg, span {
    align-self: center;
    &:nth-child(odd) {
      font-size: 2em;
    }
  }
}
</style>
